...
public:
    ...
    int8_t readSensor(void) {
        ...
        ax = (float)(_axcounts) * _accelScale;
        ay = (float)(_aycounts) * _accelScale;
        az = (float)(_azcounts) * _accelScale;
        gx = ((float)(_gxcounts) * _gyroScale);
        gy = ((float)(_gycounts) * _gyroScale);
        gz = ((float)(_gzcounts) * _gyroScale);

        ax -= _accelBias[0];
        ay -= _accelBias[1];
        az -= _accelBias[2];

        ax = _accelFactor[0] * ax + _accelFactor[1] * ay + _accelFactor[2] * az;
        ay = _accelFactor[3] * ax + _accelFactor[4] * ay + _accelFactor[5] * az;
        az = _accelFactor[6] * ax + _accelFactor[7] * ay + _accelFactor[8] * az;

        gx -= _gyroBias[0];
        gy -= _gyroBias[1];
        gz -= _gyroBias[2];

        /* Transform the magnetometer to align with the Accelerometer and Gyroscope Axis */
        hx = ((float)_hxcounts) * _magScaleX;
        hy = ((float)_hycounts) * _magScaleY;
        hz = ((float)_hzcounts) * _magScaleZ;
...
private:
    ...
    float_prec _gyroBias[3] = {-0.05077615,  0.02999995, -0.02002286};
    float_prec _magBias[3] = {26.97756877, 46.59086772, 23.41337589};
    float_prec _accelBias[3] = {0.0138495, 0.0009573, -0.0052265};
    float_prec _accelFactor[9] = {1.0046, -0.0079891, 0.0032781,
                                     -0.0079891, 1.0066, 0.018602,
                                     0.0052781, 0.021602, 0.99542};
